...
#   print(f"Train numbers:{len(dataset)}")
  print('Train numbers: ' + str(len(dataset)))
  ...
  for epoch in range(opt.niter):
    ...
    torch.save(model,opt.outf + '/GoogLeNet_epoch_' + str(epoch + 1) + '.pth')
#   print(f"Model save to '{opt.outf}'.")
  print('Model save to ' + opt.outf + '.')


def test():
  if torch.cuda.is_available():
    # model = torch.load(f'{opt.outf}/GoogLeNet_epoch_{opt.niter}.pth')
    model = torch.load(opt.outf + '/GoogLeNet_epoch_' + str(opt.niter) + '.pth')
  else:
    # model = torch.load(f'{opt.outf}/GoogLeNet_epoch_{opt.niter}.pth', map_location="cpu")
    model = torch.load(opt.outf + '/GoogLeNet_epoch_' + str(opt.niter) + '.pth' , map_location="cpu")
  model.eval()
